/**
 * @file   MsgGen.cpp
 * @brief  Message generator functions (source file)
...
void
ipfix_drec::append_string(const std::uint8_t *value, uint16_t str_len) {
    if (str_len == SIZE_AUTO) {
        throw std::invalid_argument("SIZE_AUTO is not supported!");
    }

    uint8_t *mem = mem_reserve(str_len);

    if (fds_set_string(mem, str_len, reinterpret_cast<const char *>(value)) != FDS_OK) {
        throw std::invalid_argument("fds_set_string() failed!");
    }
}
...
void
ipfix_drec::append_ip(uint8_t *ip, enum fds_iemgr_element_type type) {
    size_t size;
    if (type == FDS_ET_IPV4_ADDRESS) {
        size = 4;
    } else if (type == FDS_ET_IPV6_ADDRESS) {
        size = 16;
    } else {
        throw std::invalid_argument("Invalid type of IP address!");
    }

    uint8_t *mem = mem_reserve(size);
    if (fds_set_ip(mem, size, ip) != FDS_OK) {
        throw std::invalid_argument("fds_set_ip() failed!");
    }
}
...
void
ipfix_drec::append_mac(const uint8_t *mac) {
    size_t size = 6;
    uint8_t *mem = mem_reserve(size);
    if (fds_set_mac(mem, size, mac) != FDS_OK) {
        throw std::invalid_argument("fds_set_mac() failed!");
    }
}
...
void ipfix_drec::append_field(struct fds_drec_field &field) {

    uint16_t size = field.info->length;
    if (size == SIZE_VAR) {
        size = field.size;
        var_header(size);
    }
    uint8_t *dst = mem_reserve(size);
    std::memcpy(dst, field.data, size);
}
